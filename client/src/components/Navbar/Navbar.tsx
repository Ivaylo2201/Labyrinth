    ...
    <div>
      <nav className="bg-zinc-900 w-full p-3 flex flex-col lg:flex-row ">
        <span className="flex justify-between lg:w-1/2">
          <a href={"/"} className="w-2/5 flex direction-col items-center gap-2">
            <img src={logo} alt="logo" className="w-11" />
            <h1 className="text-2xl font-bold text-white">Labyrinth</h1>
          </a>
          <button className={`text-white lg:hidden `} onClick={openCloseMenu}>
            {iconMenu}
          </button>
        </span>
        <div
          ...
          } lg:flex-row lg:justify-between ${menuIsOpen ? "lg:flex" : "lg:flex"} lg:w-full `}
        >
            ...
            {isAuthenticated && (
              <>
                ...
                <NavButton
                  ...
                  className="primary-button"
                />
                <NavButton name="Profile" location="/profile" className="primary-button" />
              </>
            )}
            <NavButton name="Admin" location="/admin" className="primary-button" />
          </div>
