...
export default function Home() {
  const [action, setAction] = useState("Choose a action"); // State for the action select
  const [property, setProperty] = useState("Choose a property"); // State for the property select
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden flex items-center justify-center">
      ...
      <div className="w-full h-full flex items-center justify-center flex-col">
        ...
        <h4 className="text-2xl font-light relative text-white">
          Labyrinth helps people find the perfect estate for a better life
        </h4>
        <div className="relative inset-0 flex gap-1 w-3/6 h-12 bg-slate-50 p-2 rounded-full my-10 justify-between">
          <div className="relative">
            <select
              className="h-8 w-full text-sm text-gray-500 bg-transparent appearance-none mx-2 pr-8 focus:outline-none"
              value={action} // Set the value of the select
              onChange={(e) => setAction(e.target.value)} // Update the state on change
            >
              <option value="Choose a action" disabled>Choose an action</option>
              <option value="buy">For buy</option>
          ...
          <div className="relative">
            <select
              className="h-8 w-full text-sm text-gray-500 bg-transparent appearance-none pr-8 focus:outline-none"
              value={property} // Set the value of the select
              onChange={(e) => setProperty(e.target.value)} // Update the state on change
            >
              <option value="Choose a property" disabled>Choose a property</option>
              <option value="apartment">Apartment</option>
              <option value="house">House</option>
              <option value="garage">Garage</option>
              <option value="office">Office</option>
